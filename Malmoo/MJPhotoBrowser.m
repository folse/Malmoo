...
-(void)DeleteThisImage:(NSInteger)imageIndex
{
    if ( imageIndex == 0 ) {
        _currentPhotoIndex = 1;
    }else if ( imageIndex == _currentPhotoIndex ) {
        _currentPhotoIndex = _currentPhotoIndex - 1;
    }else{
        _currentPhotoIndex = _currentPhotoIndex - 1;
    }
    
    [_photos removeObjectAtIndex: imageIndex];
...
- (void)showPhotos
{
    ...
    if (firstIndex >= _photos.count) firstIndex = 0;
    if (lastIndex < 0) lastIndex = 0;
    if (lastIndex >= _photos.count) lastIndex = 0;
...
- (void)loadImageNearIndex:(int)index
{
    if (index > 0) {
        ...
        [[SDWebImageManager new] downloadImageWithURL:photo.url options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
